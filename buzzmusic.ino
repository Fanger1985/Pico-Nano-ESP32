...
void loop() {
  // Play the extended Super Mario theme
  for (int thisNote = 0; thisNote < sizeof(marioMelody)/sizeof(marioMelody[0]); thisNote++) {
  ...
  delay(1500); // Delay between songs

  // Play the Imperial March
  for (int thisNote = 0; thisNote < sizeof(imperialMelody)/sizeof(imperialMelody[0]); thisNote++) {
    int noteDuration = imperialNoteDurations[thisNote];
    tone(buzzerPin, imperialMelody[thisNote], noteDuration);
    int pauseBetweenNotes = (noteDuration <= 500) ? noteDuration * 1.30 : noteDuration;
    delay(pauseBetweenNotes);
    noTone(buzzerPin);
  }

  delay(3000); // Delay before replaying the loop
}
